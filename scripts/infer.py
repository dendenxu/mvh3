...
def main():
    ...
    parser.add_argument("--request", required=True, action="append",
    ...
    parser.add_argument("--verify-init", action="store_true",
                        help="Compare every static-camera prediction with base weights")
    ...
    run(cfg, request_path=args.request, output=args.output, checkpoint=args.checkpoint, protocol=args.protocol,
        seed=args.seed, steps=args.steps, camera=not args.no_camera, verify_init=args.verify_init)
