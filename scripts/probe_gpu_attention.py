...
from mvh3.camera import precompute_camera
from mvh3.masking import CLEAN, CONDITION, NOISY, TokenLayout
from mvh3.transformer import MiniMaxH3TransformerBlock
...
def main():
    ...
    grad_relative = [((a.float()-b.float()).norm() / b.float().norm()).item() for a, b in zip(sparse_grad, dense_grad)]
    ...
    report = {"status": "passed", "device": args.device, "dtype": "bfloat16", "tokens": size,
              "dense_sparse_max_abs": (output.float() - dense.float()).abs().max().item(),
              "gradient_relative_errors": grad_relative, "three_layer_future_invariance": "exact"}
    if args.output:
