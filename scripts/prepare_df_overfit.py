...
def main():
    ...
    view.update(caption_scene=scene, caption_motions=motions,
                caption_source_frames=view["source_frames"], prompt=scene)
    ...
    for name in ("generation_chunks", "clean_prefix_chunks", "texts_by_bd", "texts", "text_tag_specs", "caption_specs"):
        view.pop(name, None)
    ...
    if groups.get_rank() == 0:
        ...
        metadata.update(caption_policy="bd_overlap", caption_bank=list(captions),
