...
from h3.data import pad_video, read_parquet_clip, temporal_layout
...
def main():
    ...
    parser.add_argument("--reuse-text-from",
                        type=Path,
                        help="Directory of prior full-encoder features for the same source caption")
    ...
    diffusers_version = "local-d30c748"
    from h3.vendor.convert_minimax_h3 import MINIMAX_H3_VIDEO_VAE_CONFIG, convert_video_vae

    converted = args.vae_cache or args.output / "vae"
    if not (converted / "config.json").exists():
        log("Converting the original full video VAE")
        convert_video_vae(str(args.checkpoint), str(converted), MINIMAX_H3_VIDEO_VAE_CONFIG, diffusers_version,
                          2 * 1024**3)
    vae = AutoencoderKLMiniMaxH3.from_pretrained(converted, torch_dtype=torch.float32,
    ...
    encoded = {}
    for stage, frames, selected in (("short_mono", args.short_frames, [0]), ("long_multiview", args.frames,
        ...
        encoded[stage] = {
            ...
            "camera_pose": clip["pose"][selected][:, layout.camera_frames],
            "rotary_frames": layout.rotary_frames,
            "valid_frames": layout.valid,
    ...
    if args.reuse_text_from:
        ...
        if (report["status"] != "complete" or report["caption"] != clip["caption"]
                or Path(report["checkpoint"]).resolve() != args.checkpoint.resolve() or report["text_layer"] != 50):
            raise ValueError("Cached text must come from the same full encoder and exact source caption")
        prompt = torch.load(args.reuse_text_from / "short_mono.pt", map_location="cpu",
                            weights_only=True)["prompt_embeds"]
    ...
    report = {
        ...
        "row": args.row,
        "source_paths": clip["source_paths"],
        "source_view_count": clip["source_view_count"],
        "selected_views": views,
        "sample_indices": clip["sample_indices"],
        "caption": clip["caption"],
        "stages": {
            stage: {
                "shape": list(data["latents"].shape),
                "source_frames": data["source_frames"],
                "padded_frames": data["padded_frames"],
                "valid_latents": int(data["valid_frames"].sum())
            }
            for stage, data in encoded.items()
        },
        "text_shape": list(prompt.shape),
        "text_layer": 50,
