...
def visibility_groups(layout):
    ...
    return VisibilityGroups(torch.cat([q for q, _ in pairs]), torch.cat([k for _, k in pairs]),
                            cu_q, cu_k, max(q_sizes), max(k_sizes))


@torch.library.custom_op("mvh3::grouped_attention_backward", mutates_args=())
def grouped_backward(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                     output: torch.Tensor, grad_output: torch.Tensor, lse: torch.Tensor,
                     q_order: torch.Tensor, k_order: torch.Tensor,
                     cu_q: torch.Tensor, cu_k: torch.Tensor,
                     max_q: int, max_k: int, deterministic: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    q, k, v = (gather(x, indices) for x, indices in ((query, q_indices), (key, k_indices), (value, k_indices)))
    ...
    dq, dk, dv = _flash_attn_bwd(q, k, v, out, dout, grouped_lse,
                                cu_seqlens_q=batch_cu(cu_q, q_order.numel()),
                                cu_seqlens_k=batch_cu(cu_k, k_order.numel()),
                                max_seqlen_q=max_q, max_seqlen_k=max_k, deterministic=deterministic)
    ...
    return tuple(torch.empty_like(original).copy_(gradient.reshape(batch, size, heads, dim).transpose(1, 2))
                 for original, gradient in zip((query, key, value), restored))


@grouped_backward.register_fake
def grouped_backward_fake(query, key, value, output, grad_output, lse,
                          q_order, k_order, cu_q, cu_k, max_q, max_k, deterministic=False):
...
class GroupedFlexAttention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, block_mask, q_order, k_order, cu_q, cu_k, max_q, max_k, deterministic):
        output, auxiliary = flex_attention(query, key, value, block_mask=block_mask,
                                           kernel_options={"BACKEND": "FLASH"}, return_aux=AuxRequest(lse=True))
    ...
    def backward(ctx, grad_output):
        ...
        gradients = grouped_backward(query, key, value, output, grad_output, lse,
                                      q_order, k_order, cu_q, cu_k, ctx.max_q, ctx.max_k, ctx.deterministic)
        return (*gradients, *(None for _ in range(8)))


@torch.library.custom_op("mvh3::grouped_attention_backward_metadata", mutates_args=())
def grouped_backward_metadata(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                              output: torch.Tensor, grad_output: torch.Tensor, lse: torch.Tensor,
                              q_order: torch.Tensor, k_order: torch.Tensor,
                              cu_q: torch.Tensor, cu_k: torch.Tensor,
                              sizes: torch.Tensor, deterministic: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    # CPU launch metadata avoids both scalar graph guards and GPU synchronization.
    max_q, max_k = sizes.tolist()
    return grouped_backward(query, key, value, output, grad_output, lse,
                            q_order, k_order, cu_q, cu_k, max_q, max_k, deterministic)


@grouped_backward_metadata.register_fake
def grouped_backward_metadata_fake(query, key, value, output, grad_output, lse,
                                   q_order, k_order, cu_q, cu_k, sizes, deterministic=False):
    return torch.empty_like(query), torch.empty_like(key), torch.empty_like(value)
...
class MetadataGroupedFlexAttention(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        gradients = grouped_backward_metadata(query, key, value, output, grad_output, lse,
                                               q_order, k_order, cu_q, cu_k, sizes, ctx.deterministic)
