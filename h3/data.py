...
def read_parquet_clip(parquet, row_index, views, num_frames, fps=16, height=448, width=832):
    ...
    columns = [
        key for key in ("video_path", "caption", "pose", "num_frames", "fps", "height", "width", "frame_start")
        if key in reader.schema_arrow.names
    ]
    offset, row = 0, None
    ...
    camera[...,
           4:7] = Rotation.from_matrix(reference_rotation.T @ rotations).as_rotvec().reshape(camera.shape[:-1] + (3, ))
    camera[..., 7:10] = (camera[..., 7:10] - reference_center) @ reference_rotation
    return {
        "pixels": clips,
        "pose": torch.from_numpy(camera),
        "caption": row["caption"],
        "source_paths": source_paths,
        "source_view_count": poses.shape[0],
