...
class DiffusionObjective:
    ...
    def __init__(self, cfg):
        ...
        self.scheduler = FlowMatchScheduler(
            shift=cfg.model.timestep_shift,
            sigma_min=0,
            extra_one_step=True,
        ...
        self.boundary_index = int(
            torch.searchsorted(
                -self.scheduler.timesteps_cpu, -int(cfg.model.boundary * cfg.model.num_train_timesteps)
            )
        )
    ...
    def compute_loss(self, model, document, device, step, override=None):
        ...
        if rf:
            ...
            for record in records:
                velocity = unpatchify(
                    prediction[:, record["start"] : record["stop"]].detach(), record["shape"]
                )
        ...
        if cfg.h3.get("single_sequence", False):
            ...
            log.update(
                chunk_size_min=int(sizes.min()),
                chunk_size_max=int(sizes.max()),
    ...
    def pack(self, document, device, step=0, override=None, inference=None, evaluation_sigma=None):
        ...
        # Step 1: Draw per-block diffusion levels and per-view context noise.
        ...
        if single_sequence and document["isolated"]:
            # Independent source views draw independent diffusion levels.
            # Synchronized views share the level for a given temporal block.
        ...
        if inference is None:
            history_dropout = (
                broadcast_scoped(torch.rand((max_chunks, max_chunks), device=device), "sp")
                < cfg.history_dropout_ratio
            )
        ...
        # Step 2: Attach causal captions and fixed image conditioning.
        ...
        # Step 3: Assemble video rows and their noisy-suffix supervision.
        for i, view in enumerate(views):
            ...
            if view["condition"] is not None:
                ...
                cond_sigma = cond_sigma.clamp_max(float(cfg.h3.get("condition_noise", 0.0)))
            ...
            if inference is None:
                ...
                noisy = (1 - sigma[None, None, :, None, None]) * clean + sigma[
                    None, None, :, None, None
                ] * noise
                ...
                packer.add_video(
                    view, i, noisy, slice(None), kind, sigma, target=clean - noise, weight=frame_weights
                )
            else:
                ...
                if not inference.get("cached", False):
                    ...
                    packer.add_video(
                        ...
                        history_selected,
                        CLEAN,
                        0.0 if cfg.clean_adaln else context_levels[i],
                    )
    ...
    def sample_sigmas(self, count, device):
        ...
        high = broadcast_scoped(torch.rand((), device=device), "global").item() < 0.5
        lower, upper = (
            (0, self.boundary_index) if high else (self.boundary_index, self.cfg.model.num_train_timesteps)
        )
        indices = broadcast_scoped(torch.randint(lower, upper, (count,), device=device), "sp")
        return (
            self.scheduler.sigmas.to(device)[indices],
            self.scheduler.linear_timesteps_weights.to(device)[indices],
            high,
        )

    def condition_latents(self, document, device):
        level = float(self.cfg.h3.get("condition_noise", 0.0))
        conditions = []
