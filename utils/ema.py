...
def decay_change_preserves_history(state, decay, warmup):
    ...
    return (state["warmup"] and warmup and isinstance(updates, int) and updates >= 0
            and 0 < state["decay"] < 1 and 0 < decay < 1
...
class ShardedEMA:
    def __init__(self, model, decay, device="cpu", warmup=True):
        ...
        self.weights = {name: value.detach().to(device=device, dtype=torch.float32, copy=True)
    ...
    def parameters(model):
        return {canonical_name(name): value for name, value in model.named_parameters() if value.requires_grad}
    ...
    def current_decay(self):
        return min(self.decay, (1 + self.num_updates) / (10 + self.num_updates)) if self.warmup else self.decay
    ...
    def update(self, model):
        ...
        self.num_updates += 1
        values = [value.detach().to(device=self.weights[name].device, dtype=torch.float32)
                  for name, value in parameters.items()]
        torch._foreach_lerp_(list(self.weights.values()), values, 1 - self.current_decay)

    def state_dict(self):
        return dict(decay=self.decay, warmup=self.warmup, num_updates=self.num_updates,
    ...
    def load_state_dict(self, state, *, allow_schedule_change=False):
        ...
        if not allow_schedule_change and (state["warmup"] != self.warmup or (state["decay"] != self.decay
    ...
    def average_parameters(self, model):
        ...
        backup = {name: value.detach().to(device=self.weights[name].device, copy=True)
                  for name, value in parameters.items()}
        self.swapped = True
        try:
            with torch.no_grad():
