...
from utils.math_utils import torch_inverse_3x3


def batch_rodrigues(
    rot_vecs: torch.Tensor,  # B, N, 3
    eps: float = torch.finfo(torch.float).eps,  # noqa: B008
) -> torch.Tensor:
    ...
    for i in range(len(batch_size)):  # noqa: B007
        ident = ident[None]
...
def apply_r(vds, se3):
    ...
    vds = torch.bmm(Rs, vds[:, :, None])[
        :, :, 0
    ]  # batch matmul to apply rotation, and get (N, 3) back
    vds = vds.view(B, N, -1)
    return vds
...
def get_aspect_bounds(bounds) -> torch.Tensor:
    ...
    return torch.cat(
        [middle_point - half_long_edge, middle_point + half_long_edge], dim=-2
    )


@lru_cache
def get_ndc_transform(
    bounds: torch.Tensor, preserve_aspect_ratio: bool = False
) -> torch.Tensor:
...
def get_rigid_transform(
    poses: torch.Tensor, joints: torch.Tensor, parents: torch.Tensor
):
    # pose: B, N, 3
    # joints: B, N, 3
    # parents: B, N
    ...
    J, A = batch_rigid_transform(
        R[None], joints, parents.view(-1)
    )  # MARK: doc of this is wrong about parent
    return J, A


def get_rigid_transform_nobatch(
    poses: torch.Tensor, joints: torch.Tensor, parents: torch.Tensor
):
    # pose: N, 3
    # joints: N, 3
...
def mat2rt(A: torch.Tensor) -> torch.Tensor:
    ...
    r = transforms.quaternion_to_axis_angle(
        transforms.matrix_to_quaternion(A[..., :3, :3])
    )  # n_batch, n_points, 3
...
def screw2rt(screw: torch.Tensor) -> torch.Tensor:
    ...
    return mat2rt(
        transforms.se3_exp_map(screw.view(-1, screw.shape[-1])).permute(0, 2, 1)
    ).view(*screw.shape)
...
def pose_points_to_tpose_points(
    ppts: torch.Tensor, bw=None, A=None, A_bw=None, R_inv=None
):
...
view_points_to_world_points = (
    pose_points_to_world_points  # input w2c, inversely apply w2c
)
...
def grid_sample_A_blend_weights(nf_grid_coords, bw):
    ...
    for i in range(24):
        ...
        bw_ = F.grid_sample(
            bw[:, i : i + 1], nf_grid_coords_, padding_mode="border", align_corners=True
        )
        bw_ = bw_[:, :, 0, 0]
        bws.append(bw_)
...
def forward_node_graph(
    verts: torch.Tensor,
    graph_rt: torch.Tensor,
    graph_nodes: torch.Tensor,
    graph_bones: torch.Tensor,
    ...
    vi = verts[..., None, :].expand(
        n_batch, -1, graph_bones.shape[-1], -1
    )  # B, V, 4, 3

    pj = graph_nodes[torch.arange(n_batch)[..., None, None], graph_bones]  # B, V, 4, 3
    tj = t[
        torch.arange(n_batch)[..., None, None], graph_bones
    ]  # translation B, V, 4, 3
    Rj = R[
        torch.arange(n_batch)[..., None, None], graph_bones
    ]  # rotation B, V, 4, 3, 3
...
def bilinear_interpolation(input: torch.Tensor, shape: List[int]) -> torch.Tensor:
    ...
    return F.interpolate(
        input.permute(0, 3, 1, 2), shape, mode="bilinear", align_corners=False
    ).permute(0, 2, 3, 1)
...
def linear_sample_sum_to_one(dim, samples, device="cuda", multiplier=5.0):
    ...
    for i in range(interval - 1):
        ...
        active = active + 0.5
        next = 1 - active
        weights[i * samples_per_iter : i * samples_per_iter + samples_per_iter, i] = (
            active
        )
        weights[
            i * samples_per_iter : i * samples_per_iter + samples_per_iter, i + 1
        ] = next
