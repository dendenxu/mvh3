...
def chunkify(  # noqa: C901
    ...
    move_to_numpy: bool = True,  # will return numpy instead of tensor to avoid the memory leak
    non_blocking_move_to_cpu: bool = True,
    batch_key: str = "batch",
    inds_key: str = "chunkify_sample",
    desc: str = 'Chunkified function',
):
    ...
    def merge_ret(ret, x: torch.Tensor, sh: torch.Size, nn_dim: int):
        ...
        if len(ret) and (isinstance(ret[0], torch.Tensor) or isinstance(ret[0], np.ndarray)):
            # Stop recursion
            if isinstance(ret[0], np.ndarray):
                # MARK: MAYBE SYNC HERE
                ret = np.concatenate(ret, axis=nn_dim)
            else:
                if move_to_numpy:
                    ret = to_numpy(ret)
                    ret = np.concatenate(ret, axis=nn_dim)
                else:
                    ret = torch.cat(ret, dim=nn_dim)  # FIXME: this line causes memory leak in pytorch, don't know why
            if ignore_mismatch:
                ret = ret
            else:
                if isinstance(ret[0], np.ndarray):
                    ret = (
                        ret.reshape(*sh, *ret.shape[nn_dim + 1:])
    ...
    def wrapper(decoder: Callable[[torch.Tensor], torch.Tensor]):
        ...
        def decode(*args, **kwargs):
            ...
            if print_progress:
                pbar = tqdm(total=total_size, back=3, desc=desc)  # log previous frame
            for i in range(0, total_size, actual_size):
                ...
                if move_to_numpy and not non_blocking_move_to_cpu:
                    result = to_numpy(result)
                elif move_to_cpu:
                    result = to_cpu(result, non_blocking=non_blocking_move_to_cpu)
            ...
            # Sync the cpu movement if required
            if move_to_cpu:
