...
def weighted_sample_rays(
    ...
    ray_o, ray_d = get_rays_from_ij(
        i, j, K, R, T, use_z_depth=use_z_depth, correct_pix=correct_pix
    )  # MARK: 0.xms
...
def weighted_sample_coords(wet: torch.Tensor, n_rays: int = -1) -> torch.Tensor:
    ...
    if n_rays == -1:
        return torch.nonzero(torch.ones_like(wet[..., 0]))  # MARK: assume sorted
    ...
    if weight.sum() == weight.numel():  # ?: Will this be too slow?
        ...
        indices = torch.multinomial(
            weight, n_rays, replacement=True
        )  # B, P, # MARK: 3-5ms
