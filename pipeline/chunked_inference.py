...
def generate(model, document, negative, cfg, device, steps=None, use_cache=None):
    """Denoise each block against generated history and its causal caption.

    `generated` keeps the clean output for decoding. `history` keeps the same
    frames with configured context noise, matching what training sees. Solver
    evaluations read history caches; only the final block write updates them.
    """
    ...
    negative_cache = make_caches(
        core, cfg, enabled and cfg.use_uncond_kvcache and cfg.guidance_scale != 1, document
    )
    uncond = {
        **document,
        "views": [
            {**v, "text": negative, "texts": [(-1, negative)], "prompt": cfg.negative_prompt}
    ...
    chunk_count = max(
        int(frame_chunks[v["valid"].to(device)].max()) + 1
        for frame_chunks, v in zip(chunks, document["views"])
    )
    context_noise = (
    ...
    def recompute_history(doc, chunk):
        ...
        for previous in range(chunk):
            state = dict(
                chunk=previous,
                sigma=0.0 if cfg.clean_adaln else context_noise,
                history=history,
                current=history,
    ...
    try:
        for chunk in range(chunk_count):
            ...
            for timestep in scheduler.timesteps:
                pos_step_cache = (
                    positive_cache if positive_cache is not None else recompute_history(document, chunk)
                )
                state = dict(
                    chunk=chunk,
                ...
                if cfg.guidance_scale != 1:
                    neg_step_cache = (
                        negative_cache if negative_cache is not None else recompute_history(uncond, chunk)
                    )
                ...
                updated = scheduler.step(
                    prediction if native else -prediction, timestep, packed, return_dict=False
                )[0]
                offset = 0
                for record, part in zip(records, samples):
                    n = part.shape[1]
                    current[record["view"]][:, :, record["selected"]] = unpatchify(
                        updated[:, offset : offset + n], record["shape"]
                    )
                    offset += n
            for i, frame_chunks in enumerate(chunks):
                # Select by chunk, including decoder-support latents in the
                # final block. Crop duration only after the VAE has decoded.
