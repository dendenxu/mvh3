...
class FlowUniPCMultistepScheduler:
    ...
    def __init__(
        ...
        if solver_type not in ["bh1", "bh2"]:
            if solver_type in ["midpoint", "heun", "logrho"]:
                self.config.update(solver_type="bh2")
            else:
    ...
    def set_timesteps(
        ...
        self.timesteps = torch.from_numpy(timesteps).to(device=device, dtype=torch.int64)  # why int64
    ...
    def threshold_sample(self, sample: torch.Tensor) -> torch.Tensor:
        """
        ...
        **kwargs,
        ...
        if self.predict_x0:
            ...
            if self.config.thresholding:
                x0_pred = self.threshold_sample(x0_pred)
    ...
    def multistep_uni_p_bh_update(
        ...
        **kwargs,
        ...
        for i in range(1, order):
            ...
            alpha_si, sigma_si = self.sigma_to_alpha_sigma_t(self.sigmas[si])
        ...
        rks.append(1.0)
        rks = torch.as_tensor(rks).to(device, non_blocking=True)
        ...
        R = torch.stack(R)
        b = torch.as_tensor(b).to(device, non_blocking=True)
        ...
        if len(D1s) > 0:
            ...
            if order == 2:
                ...
                rhos_p = torch.linalg.solve(R[:-1, :-1], b[:-1]).to(device, non_blocking=True).to(x.dtype)
        else:
            D1s = None
    ...
    def multistep_uni_c_bh_update(
        ...
        **kwargs,
        ...
        if order == 1:
            rhos_c = torch.as_tensor([0.5], dtype=x.dtype).to(device, non_blocking=True)
        else:
