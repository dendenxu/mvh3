...
def joint_inputs(document, current, conditions, sigma, cfg, device, camera=True):
    """Pack one native full-sequence forward, without training chunk causality."""
    ...
    inputs, _, _, records, _ = DiffusionObjective(local).pack(document, device, inference=state)
...
def generate(model, document, negative, cfg, device, steps=None, use_cache=None, camera=True, observer=None):
    """Run released H3 Euler sampling; observer can compare each base prediction.

    Initialization parity uses this full-sequence visibility with base weights.
    The trained chunked pipeline has different visibility and is tested apart.
    """
    ...
    model.eval()
    try:
        conditions = DiffusionObjective(cfg).condition_latents(document, device)
        current = [
        ...
        for index, timestep in enumerate(scheduler.timesteps):
            inputs, records = joint_inputs(
                document, current, conditions, 1 - float(timestep), cfg, device, camera
            )
