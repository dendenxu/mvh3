...
def prepare_request(request, root, video, text, cfg, chunked=None):
    ...
    images = [torch.from_numpy(np.array(Image.open(root / v["image"]).convert("RGB"))).permute(2, 0, 1)
              if v.get("image") else None for v in specifications]
    ...
    for index, (spec, prompt) in enumerate(zip(specifications, prompts)):
        ...
        if camera.ndim != 2 or camera.shape[1] != 10 or not torch.isfinite(camera).all() or not (camera[:, :2] > 0).all():
        ...
        subframes = (starts[:, None] + torch.arange(4)).minimum(layout.camera_frames[:, None]).clamp_max(frames - 1)
        ...
        if image is not None:
            ...
            condition = dict(latent=latent, frames=condition_layout.rotary_frames, valid=condition_layout.valid,
                             pose=camera[:1], projection=projection[:1], inverse=inverse[:1])
        ...
        views.append(dict(latent=target, condition=condition, pose=camera[layout.camera_frames],
                          projection=projection, inverse=inverse, frames=layout.rotary_frames, valid=layout.valid,
                          spatial_weights=weights, fps=fps, scale=float(spec.get("scale", 1)), prompt=prompt,
                          height=h, width=w,
                          source_frames=frames, source_view=index, source_start=0))
        if encoded is not None:
            views[-1].update(text=encoded[index]["features"], text_tags=encoded[index]["tags"])
        if chunked:
            views[-1].update(caption_scene=spec.get("scene", request.get("scene", prompt)),
    ...
    if chunked:
        ...
        for view, specs in zip(document["views"], pairs):
            ...
            view.update(texts=[(chunk, value["features"]) for chunk, value in values],
