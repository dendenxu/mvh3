...
class StaticDataset(Dataset):
        ...
        **kwargs,
        ...
        if self.shape_pool:
            ...
            for mv_p, _gen_p in self.shape_pool:
                assert (
                    mv_p >= 1 and (self.height % 16 == 0) and (self.width % 16 == 0)
        ...
        if is_node_main():
            log(
                f"Creating StaticDataset from {blue(data_path)}, gen_size={gen_size}, "
        ...
        if is_node_main():
            ...
            log(
                f"[{cls_name} filter] {blue(data_path)}: "
        ...
        if is_node_main():
            log(
                f"StaticDataset init: {green(len(self.metadata))} scenes from {blue(data_path)} "
    ...
    def get_video_reader(self, idx):
        ...
        if idx not in self.video_readers:
            ...
            if isfile(video_path):
                ...
                video_files = sorted(
                    [f for f in os.listdir(video_path) if f.endswith((".mp4", ".avi", ".mov"))]
                )
                if video_files:
    ...
    def load_view(
        ...
        **aug_kwargs,
        ...
        cam_indices = self.video_to_cam_indices(frame_indices - frame_start, cam_n_vid, len(cameras))

        # Collect camera dicts for these frames and adjust K for the pre-resize
        # (same as MVGame's load_view lines 132-134)
        cams = [
            {k: torch.as_tensor(v, dtype=torch.float32).clone() for k, v in cameras[ci].items()}
    ...
    def getitem_impl(self, idx: int, **kwargs):
        ...
        if self.shape_pool_active:
            ...
            for reduced_mv in (5, 2):
                if (
                    reduced_mv in pack_factory
                    and tuple(pack_factory[reduced_mv]["pack_size"]) == my_pack_size
                ):
        ...
        per_view_inds = [
            ...
            + np.minimum(np.round(model_idx * fps_ratio).astype(np.int64), seg_end - seg_start - 1)
        ...
        if len(ratios) > 1 and bool(np.all(np.asarray(ratios) == ratios[0])):
            ...
            batched = vr.get_batch(
                np.concatenate(per_view_inds).tolist(),
                return_channel_first=True,
                return_tensor=True,
                ratio=rr,
            )

        for view_idx, ((seg_start, seg_end), ratio, x_off, y_off) in enumerate(zip(segments, ratios, xs, ys)):
            target_h = int(self.height * ratio)
            target_w = int(self.width * ratio)
            seg_frame_inds = per_view_inds[view_idx]
            view_frames = (
                batched[view_idx * total_frame_size : (view_idx + 1) * total_frame_size]
        ...
        # in dataset/loader.py restores this (frame, view) order.
        batch["frames"] = frames
