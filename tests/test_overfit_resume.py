...
def test_diagnostic_resume_checks_every_saved_state(tmp_path, corrupt):
    ...
    if corrupt == "raw":
        with torch.no_grad():
            model.weight[0, 0] += 0.01
    elif corrupt == "adamw":
        optimizer.state[model.weight]["exp_avg"][0, 0] += 0.01
    elif corrupt == "ema":
        ema.weights["weight"][0, 0] += 0.01
    checks = restored_state_checks(model, optimizer, ema, saved)
    assert checks == {key: key != corrupt for key in ("raw", "adamw", "ema")}
    decision = resume_evaluation_decision(0.0001, 0.00105, "Bounded convergence probe", [checks])
...
def test_diagnostic_continuation_preserves_strict_gate_and_requires_every_rank():
    ...
    strict = resume_evaluation_decision(0.0001, 0.00105)
    assert strict["status"] == "failed" and not strict["continue_training"]
    assert resume_evaluation_decision(0.0001, 0.0009)["continue_training"]
    diagnostic = resume_evaluation_decision(0.0001, 0.00105, "Convergence only", exact)
    assert diagnostic["status"] == "failed" and diagnostic["continue_training"]
    assert diagnostic["relative_tolerance"] == strict["relative_tolerance"] == 0.001
    assert not resume_evaluation_decision(0.0001, 0.00105, " ", exact)["continue_training"]
    exact[7]["adamw"] = False
    assert not resume_evaluation_decision(0.0001, 0.00105, "Convergence only", exact)["continue_training"]
    assert not resume_evaluation_decision(0.0001, 0.0009, "Convergence only", exact)["continue_training"]


@pytest.mark.parametrize("error", [float("nan"), float("inf"), -0.01])
def test_diagnostic_resume_never_accepts_invalid_evaluations(error):
    exact = [dict(raw=True, adamw=True, ema=True)]
    decision = resume_evaluation_decision(0.0001, error, "Convergence only", exact)
