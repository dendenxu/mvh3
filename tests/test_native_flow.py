...
def native_recipe():
    cfg = load_config(Path(__file__).resolve().parents[1] / "configs/init_wrapped.yaml")
...
def conditioned_document(frames=22):
    ...
    view["condition"] = {
        key: view[key][:1].clone() for key in ("pose", "projection", "inverse", "frames", "valid")
    }
...
def test_native_recipe_and_shifted_training_noise():
    ...
    torch.manual_seed(912)
    sigma, weight, high = DiffusionObjective(cfg).sample_sigmas(200, "cpu")
...
def test_fixed_noise_evaluation_preserves_native_flow_direction():
    objective, document = DiffusionObjective(native_recipe()), conditioned_document()
    torch.manual_seed(70000)
    first = objective.pack(document, "cpu", evaluation_sigma=0.75)
    torch.manual_seed(70000)
    second = objective.pack(document, "cpu", evaluation_sigma=0.75)
    ...
    torch.testing.assert_close(noisy + 0.75 * velocity, clean, rtol=1e-5, atol=1e-6)
    times = inputs["timestep"][inputs["timestep_indices"]]
    assert times[inputs["text_indices"]].eq(0.25).all()
...
def test_condition_noise_stays_fixed_across_denoising_and_cache_writes():
    ...
    for sigma, cache in ((0.8, False), (0.2, False), (0.0, True)):
        state = dict(
            chunk=0,
            sigma=sigma,
        ...
        torch.testing.assert_close(times, torch.full_like(times, 1 - min(sigma, 0.001)))
