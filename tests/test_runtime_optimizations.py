...
class ImageProcessor:
    ...
    def __call__(self, images, **kwargs):
        return dict(image_grid_thw=torch.tensor([[1, 2, 2]] * len(images)),
...
def test_i2v_deduplicates_conditions_and_preserves_cached_collective_participant(tmp_path, monkeypatch):
    ...
    encoder.processor = SimpleNamespace(image_processor=ImageProcessor(),
                                       create_mm_token_type_ids=lambda ids: [[0] * len(row) for row in ids])
    ...
    monkeypatch.setattr(dist, "all_gather_object", lambda output, value: output.__setitem__(slice(None), [value, []]))
    for original, cached in zip(results, encoder.i2v(requests)):
        torch.testing.assert_close(original["features"], cached["features"], rtol=0, atol=0)
    assert batches == [2, 1]
...
def test_kv_budget_splits_at_token_boundary_and_preserves_values(monkeypatch):
    ...
    value = key + 10
    layout = TokenLayout(torch.full((8,), CLEAN), torch.arange(8), torch.zeros(8, dtype=torch.long), True,
    ...
    torch.testing.assert_close(torch.cat([entry[2].chunk for entry in cache.segments]), layout.chunk, rtol=0, atol=0)


def test_cfg_distilled_cache_uses_single_stream_budget():
    model = SimpleNamespace(transformer_blocks=[None] * 50)
    cfg = SimpleNamespace(kv_gpu_budget_gb=20, kv_offload=True, guidance_scale=1,
                          kv_sink_size=0, kv_window_size=0, chunk_size=5, kv_sink_view0_only=False)
...
def test_rf_warmup_skips_history_copy_and_starts_on_absolute_boundary():
    ...
    before, cold = objective(zero_velocity, document, "cpu", 11999)
    torch.manual_seed(124)
    after, warm = objective(zero_velocity, document, "cpu", 12000)
    torch.testing.assert_close(before, after, rtol=0, atol=0)
    assert cold["rf"] is False and cold["x0"] is None
    assert warm["rf"] is True and len(warm["x0"]) == len(document["views"])
    assert cold["sigma"] == cold["sigma_min"] == cold["sigma_max"] == .5
...
def test_tracking_keeps_text_out_of_numeric_history_and_preserves_global_step():
    ...
    tracker.log({"loss": .25, "train/high": True, "data/source": "/source.parquet", "recipe": {"camera": "decomposed"}}, 128)
    assert calls == [({"step": 128, "global_step": 128, "loss": .25, "train/high": 1.}, {"step": 135})]
