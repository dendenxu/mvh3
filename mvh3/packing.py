...
from .masking import CLEAN, CONDITION, NOISY, TokenLayout
...
def patchify(latents):
    ...
    return latents.reshape(views, channels, frames, height // 2, 2, width // 2, 2).permute(2, 0, 3, 5, 1, 4, 6).reshape(1, -1, 96)


def teacher_forcing_batch(features, device, seed=42, sigma=0.5, context_noise=0.2, context_noise_std=0.1, cross_view=True):
    ...
    context_levels = (context_noise + context_noise_std * torch.randn(views, generator=generator, device=device)).clamp(0, 1)
    ...
    kinds = torch.cat((torch.full_like(text_indices, CONDITION), torch.full((video_count,), CLEAN, device=device), torch.full((video_count,), NOISY, device=device)))
    ...
    positions_video = torch.cat(((text_count + starts[frames_for_token] * (40 / features["fps"]))[:, None], spatial), dim=-1)
    ...
    inputs = dict(
        hidden_states=torch.cat((context, noisy), dim=1), audio_hidden_states=clean.new_empty(1, 0, 32),
        # H3 uses t=1 for clean data and predicts data-ward velocity.
        encoder_hidden_states=prompt, timestep=torch.cat((torch.tensor([1.0, 1 - sigma], device=device), 1 - context_levels)),
        timestep_indices=timestep_indices, token_tags=tags, position_ids=positions,
        video_indices=video_indices, audio_indices=audio_indices, text_indices=text_indices,
        camera_pose=camera_pose, camera_indices=torch.cat((torch.full_like(text_indices, -1), camera_ids, camera_ids)),
