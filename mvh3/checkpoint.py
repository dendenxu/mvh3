...
from .transformer import MiniMaxH3RotaryPosEmbed, MiniMaxH3Transformer3DModel
from .vendor.convert_minimax_h3 import (
...
def original_config(checkpoint):
    ...
    names = {
        "token_refiner_num_layers": "num_refiner_layers", "ffn_hidden_size": "ffn_dim",
        "latents_dim": "in_channels", "audio_latents_dim": "audio_in_channels",
        "timestep_input_dim": "freq_dim", "time_embed_hidden_size": "time_embed_hidden_dim",
...
def original_tensors(checkpoint, config, progress=print):
    ...
    if set(index) != set(plan):
        raise ValueError(f"Source index differs from the architecture: missing={set(plan)-set(index)}, extra={set(index)-set(plan)}")
    seen = set()
        ...
        with safe_open(directory / filename, framework="pt", device="cpu") as reader:
            for source in reader.keys():
                ...
                if source.endswith(".attn.qkv_proj.weight"):
                    tensor = reorder_interleaved_qkv(tensor, config["num_attention_heads"], config["attention_head_dim"])
                ...
                for name, value in converted:
                    expected_dtype = torch.float32 if source.startswith(MINIMAX_H3_FP32_SOURCE_PREFIXES) else torch.bfloat16
...
def load_original_transformer(checkpoint, device_for_name=None, model_class=MiniMaxH3Transformer3DModel, progress=print):
